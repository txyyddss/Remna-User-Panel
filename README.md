...
| --- | --- |
| `ADMIN_TELEGRAM_ID` | Positive Telegram user ID. This exact identity can bootstrap the dashboard before normal onboarding is complete. |
...
2. Open the Mini App as `ADMIN_TELEGRAM_ID`. Admin authorization is the validated Telegram session plus the exact environment ID; it does not depend on onboarding completion.
