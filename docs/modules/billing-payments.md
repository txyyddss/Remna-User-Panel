...
- Provider redirects arriving first leave the order pending. The return redirect carries a short-lived HMAC capability; a regular browser may poll only the matching provider/order status projection, while Telegram retains owner-scoped full-order polling. Browser return pages never receive owner, amount, checkout URL, or provider payload data.
