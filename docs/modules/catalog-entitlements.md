...
- A public catalog contains only active combos and visible, upstream-present squads. Historical records are archived rather than deleted when purchases refer to them.
...
When a new term activates, synchronization resets usage, replaces the complete internal-squad list, and applies the account-wide limit and reset strategy. At term expiry, it removes all internal squads but leaves the upstream identity ACTIVE with the fixed 2099 upstream expiry. The local term—not Remnawave's user expiry—is authoritative.
