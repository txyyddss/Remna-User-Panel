...
Catalog checkout prompts for selected activation-required squads one at a time and sends the in-memory code map once with the purchase request. The group-message reward is a progress card showing message count, reward amount, and claimed/available state. Rollover detail renders forecast metrics, including predicted rollover credit or required reduction, or the localized automatic-renewal-disabled warning.
