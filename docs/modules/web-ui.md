...
The active `Your ride` summary is an accessible flip card. Its summary button is independent from renewal and queued-cancellation controls; opening it fetches `/api/v1/purchases/{id}/rollover` fresh, shows loading/retry states, and presents the current-term and latest-reset aggregate windows, net paid TXB, saved percentage, and traffic required for the configured maximum. The interaction uses transform/opacity motion with a reduced-motion state swap and keeps the back face inert while closed.
