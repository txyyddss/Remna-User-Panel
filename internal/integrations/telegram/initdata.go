...
func (v *InitDataVerifier) Verify(raw string) (*InitData, error) {
	...
	// Telegram auth_date is generated by the client device. Allow the same
	// bounded skew as the freshness window so a clock that is a little ahead
	// does not make a valid Mini App unusable, while still rejecting timestamps
	// that are materially in the future.
	if age < -v.maxAge {
