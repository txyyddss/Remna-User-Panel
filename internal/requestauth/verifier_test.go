...
const (
	testSessionToken      = "c2Vzc2lvbi10b2tlbi0zMi1ieXRlcy0wMDAwMDAwMDA"
	testOtherSessionToken = "b3RoZXItc2Vzc2lvbi10b2tlbi0zMi1ieXRlcy0wMDA"
)
...
func TestVerifyReplayCapacityIsIsolatedPerSession(t *testing.T) {
	...
	otherKey, err := verifier.ClientKey(testOtherSessionToken)
	if err != nil {
		t.Fatalf("other ClientKey(): %v", err)
	}
	other := signedRequestForSession(t, verifier, testOtherSessionToken, "GET", "/api/v1/me", "", "CCCCCCCCCCCCCCCCCCCCCC", time.Unix(1_800_000_000, 0))
	if err := verifier.Verify(other, testOtherSessionToken, otherKey); err != nil {
