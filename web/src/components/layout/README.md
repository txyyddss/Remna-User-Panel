...
- `AppShell.vue` owns the fullscreen safe-area shell, content focus, Telegram back navigation, and the Nuxt UI `UDashboardSidebar` desktop shell. Phones keep their centered fixed bottom navigation, while desktop exposes the complete member and administrator route hierarchy without duplicate Home action or Around TX blocks.
