...
- `AppShell.vue` owns the fullscreen safe-area shell, member navigation, content focus, and Telegram back navigation. The shared shell reserves a minimum mobile top boundary and combines Telegram/system top insets so native header controls cannot cover page content. Fullscreen greetings prefer the app-managed username over the Telegram username. Mobile primary navigation uses explicit Vue Router button actions without a native `href`, preventing Telegram WebView from treating internal navigation as a document load. The member navigation contains only route destinations; locale selection lives in the Home footer.
