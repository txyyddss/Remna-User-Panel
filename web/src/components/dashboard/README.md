...
- `UsagePanel.vue` shows current-term traffic as an accessible SVG graph and opens the date-bounded per-node detail; `TrafficUsageDetails.vue` owns its UTC range controls and `TrafficNodeChart.vue` renders each node's daily usage as a compact chart. `EntitlementSummary.vue` presents the active ride, reset cadence, a term slider, a separated renewal total/date block, and owner-only queued cancellation with refund feedback.
