...
- `UsagePanel.vue` keeps the current-term limit meter, adds the interactive `TrafficUsageBar.vue` node distribution bar, shows the accessible SVG graph, and opens the date-bounded per-node detail; `TrafficUsageDetails.vue` owns its UTC range controls and `TrafficNodeChart.vue` renders each node's daily usage as a compact chart. `TrafficUsageBar.vue` keeps labels hidden until hover, focus, or activation, then exposes node name, country, multiplier, bytes, and share. `EntitlementSummary.vue` presents the active ride, reset cadence, a term slider, a separated renewal total/date block, and owner-only queued cancellation with refund feedback.
