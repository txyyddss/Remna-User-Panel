...
<template>
  <section class="section-block usage-section">
    <div class="section-heading">
      <h2>Usage</h2>
      <span class="section-heading__meta">{{ stale ? 'Last known data' : 'Live period' }}</span>
    </div>

    <InlineNotice v-if="stale" tone="warning">
      {{ warning || 'Remnawave is temporarily unavailable.' }} Showing data from {{ formatDateTime(fetchedAt) }}.
    </InlineNotice>
    ...
    <div class="usage-layout">
      ...
      <dl class="usage-totals">
        <div>
          <dt><PhChartDonut :size="18" /> This term</dt>
          <dd>{{ formatBytes(statistics.usedTrafficBytes) }}</dd>
          <small>of {{ formatBytes(statistics.trafficLimitBytes) }}</small>
        </div>
        <div>
          <dt><PhDatabase :size="18" /> Lifetime</dt>
