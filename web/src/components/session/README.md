...
- `SessionEntrance.vue` accepts the session `status` and a default page slot. It uses a serialized Motion presence handoff so the loading cover exits before the full-height page enters, avoiding overlapping startup layouts while preserving the reveal. It owns no authentication or navigation decisions.
- `useLoadingSequence.ts` derives loading/cover visibility from session status. Successful fast loads finish the 1.2-second introduction; slow loads retain the settled wordmark until ready. Errors bypass the introduction hold. Reduced motion skips the hold; timers and media listeners are disposed with the component.
