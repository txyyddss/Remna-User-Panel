...
- `AffiliateSummary.vue` presents the server-built link and commission metrics with light copy feedback.
- `AffiliateTierProgress.vue` renders current and next tier progress accessibly.
- `AffiliateReferralList.vue` renders Telegram first/last names and one status per fixed five-row server page with semantic selection feedback for pagination.
