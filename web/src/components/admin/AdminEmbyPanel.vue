...
<template>
  <section class="admin-panel">
    ...
    <div v-else class="admin-list"><article v-for="account in items" :key="account.id" class="admin-list-row"><span class="feature-icon feature-icon--small"><PhMonitorPlay :size="18" /></span><div><strong>{{ account.username }}</strong><small>{{ account.maxParentalRating === null ? 'No rating ceiling' : `Rating value ${account.maxParentalRating}` }} · {{ account.libraryIds.length }} libraries · updated {{ formatDateTime(account.updatedAt) }}</small></div><StatusBadge :tone="account.status === 'active' ? 'success' : account.status === 'failed' ? 'danger' : 'warning'" :label="account.status" /><button v-if="account.retryable" class="button button--secondary button--small" type="button" :disabled="busyId === account.id" @click="retry(account.id)">{{ busyId === account.id ? 'Retrying' : 'Retry safely' }}</button></article><div v-if="!items.length" class="empty-inline"><div><h3>No Emby accounts</h3><p>Member setup records will appear here.</p></div></div></div>
  </section>
</template>
