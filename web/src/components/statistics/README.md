...
- `StatisticsOverview.vue` centers the weekly added-user total in the generic user-conversion donut and combines usage, spend, rollover, activity, squad, and database KPIs.
...
- `StatisticsPaymentDonut.vue` renders one generic terminal-state donut across EZPay, BEPusdt, and Telegram Stars.
- `StatisticsCharts.test.ts` covers donut, labeled-pie, and normalized stacked-bar semantics.
- `StatisticsPaymentDonut.test.ts` covers terminal payment-state labels and the explicit no-terminal-payment state.
