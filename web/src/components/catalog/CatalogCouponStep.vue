...
<template>
  <section class="catalog-coupon-step">
    ...
    <form class="catalog-coupon-form" @submit.prevent="submit">
      ...
      <UButton type="submit" color="neutral" variant="outline" :loading="redeeming" :disabled="redeeming || !code" :label="redeeming ? $t('coupons.redeeming') : $t('coupons.redeem')" />
    </form>
    <InlineNotice v-if="message" tone="success">{{ message }}</InlineNotice>
    <InlineNotice v-if="error" tone="warning">{{ error }}</InlineNotice>
    <div class="catalog-coupon-list">
      <UButton class="catalog-coupon-choice" :class="{ 'catalog-coupon-choice--selected': couponGrantId === null }" color="neutral" variant="ghost" :aria-pressed="couponGrantId === null" @click="couponGrantId = null">
      ...
      <div v-for="grant in props.coupons" :key="grant.id" class="catalog-coupon-choice-row">
        <UButton class="catalog-coupon-choice" :class="{ 'catalog-coupon-choice--selected': couponGrantId === grant.id, 'catalog-coupon-choice--ineligible': !eligibleIds.includes(grant.id) }" color="neutral" variant="ghost" :aria-pressed="couponGrantId === grant.id" :disabled="!eligibleIds.includes(grant.id)" @click="couponGrantId = grant.id">
        ...
        <UButton color="error" variant="ghost" square icon="i-ph-trash" :aria-label="$t('coupons.discardTitle', { name: grant.coupon.name })" @click="pendingDiscard = grant" />
      </div>
    </div>
