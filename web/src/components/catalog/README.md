...
- `ComboOption.vue` and `SquadSelector.vue` collect the core-combo and optional-squad selections; customer-facing occupancy is rounded to a whole percentage without exposing exact stock counts. Only full paid add-ons are greyed, disabled, and labeled with localized Full copy while bundled squads remain server-validated selections.
